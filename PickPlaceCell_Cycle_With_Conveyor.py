...
class TaskPlanner:
    ...
    def start(self, abb_flag, ur_flag, melfa_flag):
        ...
        # offset //pick start point----IRB1200
        # in
        # arg_list_2_1[0][2][1][2]=arg_list_2_1[0][2][1][2]-self.abb_offset_pick[self.abb_index[0]]
        # # out
        # arg_list_2_1[2][2][0][2]=arg_list_2_1[2][2][0][2]+self.abb_offset_pick[self.abb_index[0]]
        # # in
        # arg_list_2_1[6][2][2][2]=arg_list_2_1[6][2][2][2]-self.abb_offset_pick[self.abb_index[1]]
        # #out
        # arg_list_2_2[1][2][0][2]=arg_list_2_2[1][2][0][2]+self.abb_offset_pick[self.abb_index[1]]
        # #in
        # arg_list_2_2[5][2][2][2]=arg_list_2_2[5][2][2][2]-self.abb_offset_pick[self.abb_index[2]]
        # #out
        # arg_list_2_3[1][2][0][2]=arg_list_2_3[1][2][0][2]+self.abb_offset_pick[self.abb_index[2]]

        # # offset //pick start point----UR5
        # # in
        # arg_list_0_1[0][2][1][2]=arg_list_0_1[0][2][1][2]-self.ur5_offset_pick[self.ur_index[0]]
        # # out
        # arg_list_0_1[2][2][0][2]=arg_list_0_1[2][2][0][2]+self.ur5_offset_pick[self.ur_index[0]]
        # # in
        # arg_list_0_1[6][2][2][2]=arg_list_0_1[6][2][2][2]-self.ur5_offset_pick[self.ur_index[1]]
        # #out
        # arg_list_0_2[1][2][0][2]=arg_list_0_2[1][2][0][2]+self.ur5_offset_pick[self.ur_index[1]]
        # #in
        # arg_list_0_2[5][2][2][2]=arg_list_0_2[5][2][2][2]-self.ur5_offset_pick[self.ur_index[2]]
        # #out
        # arg_list_0_3[1][2][0][2]=arg_list_0_3[1][2][0][2]+self.ur5_offset_pick[self.ur_index[2]]

        # # offset //pick start point----melfa
        # # in
        # arg_list_1_1[0][2][1][2]=arg_list_1_1[0][2][1][2]-self.melfa_offset_pick[self.melfa_index[0]]
        # # out
        # arg_list_1_1[2][2][0][2]=arg_list_1_1[2][2][0][2]+self.melfa_offset_pick[self.melfa_index[0]]
        # # in
        # arg_list_1_1[6][2][2][2]=arg_list_1_1[6][2][2][2]-self.melfa_offset_pick[self.melfa_index[1]]
        # #out
        # arg_list_1_2[1][2][0][2]=arg_list_1_2[1][2][0][2]+self.melfa_offset_pick[self.melfa_index[1]]
        # #in
        # arg_list_1_2[5][2][2][2]=arg_list_1_2[5][2][2][2]-self.melfa_offset_pick[self.melfa_index[2]]
        # #out
        # arg_list_1_3[1][2][0][2]=arg_list_1_3[1][2][0][2]+self.melfa_offset_pick[self.melfa_index[2]]
    ...
    def unpack(self, abb_flag, ur_flag, melfa_flag,abb_put_back_index,ur_put_back_index,melfa_put_back_index,):
...
def UR5_Set_Tray(rtr, tray):
    ...
    goal_types = ['target', 'target']
    goal_values = [home, tray_pick_approach]
    move_types = ['roadmap','roadmap']
...
def generate_put_cube_to_tray_task(robot, cube, dest):
    ...
    if (robot == 'UR5' and cube == task_planner.ur_index[0]) or (robot == 'IRB1200' and cube == task_planner.abb_index[0]) or (robot == 'RV5AS' and cube == task_planner.melfa_index[0]):
        ...
        if robot == 'UR5':
            ...
            goal_values = [cube_pick, [0, 0, -48, 0, 0, 0]]####-38   ---   -43   
        elif robot == 'RV5AS':
...
# //pick---->place
    goal_types = ['pose', 'target', 'pose']
    if robot == 'UR5':
        goal_values = [[0, 0, 48, 0, 0, 0], cube_place, [0,0,-38,0,0,0]]    
    elif robot == 'RV5AS':
        goal_values = [[0, 0, 48, 0, 0, 0], cube_place, [0,0,-36,0,0,0]]
    elif robot == 'IRB1200':
        goal_values = [[0, 0, 45, 0, 0, 0], cube_place, [0,0,-35,0,0,0]]
    move_types = ['direct','roadmap', 'direct']
    rel_pose_flags = [True, False, True]
    smoothing_values = [10.0, 10.0, 10.0]
    col_check_flags = [False, True, False]
    ...
    #    place---->home---movement
        ...
        if robot == 'UR5':
            ...
            goal_values = [[0,0,32,0,0,0], cube_next, [0, 0, -45, 0, 0, 0]]
...
def main():
    ...
    try:
        ...
        ## Uncomment this section if you would like to check if there is a tray in tray 1 position before running the task

        # if workcell_io.plc.read_input(0,d_in['tray_1']) != True:
        #    task_planner.log('Please place a tray to Tray 1 loading area before starting the task')
        #    return
        ...
        handle_response(task_planner.cmd.set_interrupt_behavior('MIT', 1, 3))
        ...
        handle_response(task_planner.cmd.set_alternate_location('MIT', 0, 0, 'melfa_standby'))
        handle_response(task_planner.cmd.set_alternate_location('ABB1200', 0, 0, 'abb_standby')) 
        handle_response(task_planner.cmd.set_alternate_location('UR5', 0, 0, 'ur_vacuum_home')) 
        ...
        task_planner.SetABBCubePickIndex([4,5,6])
        ...
        ret=pick_tray_to_conveyor()
        if ret != 0:
            return 
        
        # pick tray2 
        ret = UR5_Set_Tray(task_planner, 2) ## currently only tray 1 position is in the project
        if ret != 0:
            return    
        ret = task_planner.SetObjectStates(['fulltray_on_front_conveyor'], ['Suppressed'])
        if ret != 0:
            return ret
        ...
        task_planner.SetURCubePickIndex([4,5,6])
        task_planner.SetMelfaCubePickIndex([4,5,6])
        task_planner.start(True, True, True) #uncomment to move all robots: still testing in simulation and comment the upper section doing the pick and place in sequence
        ...
        ret = task_planner.SetObjectStates(['fulltray_on_front_conveyor_in'], ['ready'])
        
        if ret != 0:
            return 

        ret=pick_tray_from_conveyor()
        if ret != 0:
            return 
        
        task_planner.unpack(True,True,True,[1,2,3],[1,2,3],[1,2,3])
        ...
        task_planner.unpack(True,True,True,[4,5,6],[4,5,6],[4,5,6])

        ret=pick_tray_to_stroage(task_planner, 2)
        if ret != 0:
            return 
...
def pick_tray_to_stroage(rtr, tray):
    ...
    goal_types = ['pose', 'target', 'pose']
    goal_values = [[0, 0, 60, 0, 0, 0], tray_pick_approach, [0,0,-35,0,0,0]]
    move_types = ['direct','roadmap', 'direct']
    rel_pose_flags = [True, False, True]
    smoothing_values = [10.0, 10.0, 10.0]
    col_check_flags = [False, True, False]
    ...
    goal_types = ['pose', 'target']
    goal_values = [[0, 0, 35, 0, 0, 0], 'ur5_actuator_home']
    move_types = ['direct','roadmap']
    rel_pose_flags = [True, False]
    smoothing_values = [10.0, 10.0]
    col_check_flags = [False, True]
