...
lock = Lock()
# git remote add rpc_2.0  git@github.com:kevin-ur/python_manager2.0.git
...
class TaskPlanner:
    ...
    def __init__(self, rtc_ip_address, fp):
        ...
        ret = self.SetObjectStates(objects, states)
        if ret != 0:
            return ret
        ret = self.SetRobotPreset('IRB1300', 'vaccum_pick')
        if ret != 0:
            return ret
        self.SetSpeedFactor(0.2)
        ret = self.Move('IRB1300', 'target', 'irb1300_home', 'roadmap')
        if ret != 0:
            return ret
    ...
    def EnterOperationMode(self):
        ...
        if code != 0:
            ...
            print ('[task_planner] Return Code: ' + str(code))
            return code
...
abb_robot_ip="192.168.1.2"
abbio=ABB_IO_INTERFACE(abb_robot_ip,1025,simulation=True)
##Start
def InitialIRB1300Objects(rtr):
    objects = ['fulltray1_on_pallet','fulltray2_on_pallet','fulltray3_on_pallet',
              'fulltray4_on_pallet',"fulltray_on_end_left","fulltray_on_end_right"]
...
def main():
    ...
    robot_name="IRB1300"
    try:   
        task_planner.SetSpeedFactor(0.1)
        handle_response(task_planner.cmd.set_interrupt_behavior(robot_name, 1, 3))

        if InitialIRB1300Objects(task_planner) != 0:
            return
        while True:
             if (abbio.query_di01()=="TRUE"):
                objects = ["fulltray_on_end_left","fulltray_on_end_right"]
                ...
                if ret != 0:
                    return ret
                ret = task_planner.SetRobotPreset('IRB1300', 'vaccum_pick')
                if ret != 0:
                    return ret
                # pick full tray to conveyor
                p_pick_full_tray_irb1300_tweak=[233.547,-2020.846,22.46,179,0,91.27]
                
              
                p_place_full_tray_irb1300_tweak=[-3.662,-2008.929,23,180,0,91.28]

                if task_planner.Move(robot_name, 'target', 'irb1300_pick_approch', 'roadmap') != 0:
                    return 
                if task_planner.Move(robot_name, 'pose', p_pick_full_tray_irb1300_tweak, 'direct',relative_pose=False,blind_move=True) != 0:
                    return         
                abbio.vacuum_on()

                if task_planner.SetObjectStates(['fulltray_on_end_right'],['Suppressed']) != 0:
                    return 
                if task_planner.SetRobotPreset('IRB1300', 'vaccum_place') != 0:
                    return
              
                if task_planner.Move(robot_name, 'pose', [0,0,110,0,0,0], 'direct',relative_pose=True,blind_move=True) != 0:
                    return            

                if task_planner.Move(robot_name, 'target', 'irb1300_place_approch', 'roadmap',smooth=100) != 0:
                    return       

                if task_planner.Move(robot_name, 'pose', p_place_full_tray_irb1300_tweak, 'direct',relative_pose=False,blind_move=True) != 0:
                    return       

                abbio.vacuum_off()

                if task_planner.SetRobotPreset('IRB1300', 'vaccum_pick') != 0:
                    return 

                if task_planner.SetObjectStates(['fulltray_on_end_left'],['ready']) != 0:
                    return 


                if task_planner.Move(robot_name, 'pose', [0,0,50,0,0,0], 'direct',relative_pose=True,blind_move=True) != 0:
                    return          
                if task_planner.Move(robot_name, 'target', 'irb1300_home', 'roadmap') != 0:
                    return        

             else:
                objects = ["fulltray_on_end_left","fulltray_on_end_right"]
                states = ['Suppressed', 'ready']
                if task_planner.SetObjectStates(objects, states) != 0:
                    return
                pallet_target_list=['full_tray_Point1','full_tray_Point2','full_tray_Point3','full_tray_Point4'] 
                pallet_object_list=['fulltray1_on_pallet','fulltray2_on_pallet','fulltray3_on_pallet','fulltray4_on_pallet']
                ...
                p_place_full_tray_irb1300_tweak=[-26.992,-2016.469,23.346,180,0,91.28]
                if task_planner.Move(robot_name, 'target', 'irb1300_pick_approch', 'roadmap') != 0:
                    return 
                if task_planner.Move(robot_name, 'pose', p_pick_full_tray_irb1300_tweak, 'direct',relative_pose=False,blind_move=True) != 0:
                    return         
                ...
                p_pick_full_tray_irb1300_tweak_retreat=[233.547,-2020.846,44.46,180,0,91.27]
                
                if task_planner.Move(robot_name, 'pose', p_pick_full_tray_irb1300_tweak_retreat, 'direct',relative_pose=False,blind_move=True) != 0:
                    return            
            
                if task_planner.Move(robot_name, 'target', pallet_target_list[Index_Place], 'roadmap',smooth=500) != 0:
                    return       
                
                if task_planner.Move(robot_name, 'pose', [0,0,-63,0,0,0], 'direct',relative_pose=True,blind_move=True) != 0:
                    return     
                # abbio.vacuum_off()
                ...
                if task_planner.SetObjectStates([pallet_object_list[Index_Place]],['ready']) != 0:
                    return 

                if task_planner.Move(robot_name, 'pose', [0,0,10,0,0,0], 'direct',relative_pose=True,blind_move=True) != 0:
                    return     
                # abbio.vacuum_off()
